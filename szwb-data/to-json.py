...
file_list = ['szwb-data/trs/szwb-0.txt', 'szwb-data/trs/szwb-1.txt']
...
for date in record_dict:
    index_in_date = 0
    for record in record_dict[date]:
        index_all = record['date'].replace('-', '') + format(index_in_date, '03d')
        json_dict[index_all] = record
        index_in_date = index_in_date + 1


print('begin dumping to json')
json.dump(json_dict, json_file, ensure_ascii=False, sort_keys=True, indent= 4, separators=(',' ':'))